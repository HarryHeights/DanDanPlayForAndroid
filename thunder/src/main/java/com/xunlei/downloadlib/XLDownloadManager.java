...
public class XLDownloadManager {
    ...
    private static class ManagerHolder{
        private static XLDownloadManager xlDownloadManager = new XLDownloadManager();
    }

    public static XLDownloadManager getInstance(){
        return ManagerHolder.xlDownloadManager;
    }
        ...
        synchronized (this) {
            ...
            if (!(context == null || initParam == null || !initParam.checkMemberVar())) {
                ...
                if (mDownloadManagerState == XLManagerStatus.MANAGER_RUNNING) {
                    LogUtils.i(TAG, "XLDownloadManager is already init");
                } else if (this.mLoader != null) {
                    this.mAppkeyChecker = new XLAppKeyChecker(context, initParam.mAppKey);
                    if (this.mAppkeyChecker.verify()) {
                        LogUtils.i(TAG, "appKey check successful");
                        String soAppKey = this.mAppkeyChecker.getSoAppKey();
                        String peerid = getPeerid();
                        String guid = getGuid();
                        LogUtils.i(TAG, "Peerid:" + new String(Base64.encode(peerid.getBytes(), 0)));
                        LogUtils.i(TAG, "Guid:" + new String(Base64.encode(guid.getBytes(), 0)));
                        if (mAllowExecution) {
                        ...
                        LogUtils.i(TAG, "appKey check failed");
                        i = 9901;
                    }
                }
            }
    ...
    public synchronized int uninit() {
        ...
        synchronized (this) {
            if (mRunningRefCount != 0) {
                LogUtils.d("some function of XLDownloadManager is running, uninit failed!");
    ...
    int notifyNetWorkType(int i, XLLoader xLLoader) {
        ...
        if (mDownloadManagerState == XLManagerStatus.MANAGER_RUNNING && xLLoader != null) {
            try {
                ...
                LogUtils.e("notifyNetWorkType failed," + e.getMessage());
            }
        }
        return i2;
    }
    ...
    public int addServerResource(long j, ServerResourceParam serverResourceParam) {
        ...
        if (serverResourceParam != null && serverResourceParam.checkMemberVar()) {
            increRefCount();
            if (mDownloadManagerState == XLManagerStatus.MANAGER_RUNNING && this.mLoader != null) {
                LogUtils.d("respara.mUrl=" + serverResourceParam.mUrl);
    ...
    int notifyWifiBSSID(String str, XLLoader xLLoader) {
        ...
        if (mDownloadManagerState == XLManagerStatus.MANAGER_RUNNING && xLLoader != null) {
            ...
            try {
                ...
                LogUtils.e("setNotifyWifiBSSID failed," + e.getMessage());
            }
        }
        return i;
    }
    ...
    private String getGuid() {
        ...
        if (guidInfo.mType != GUID_TYPE.ALL) {
            LogUtils.d("Start the GetGuidTimer");
            startGetGuidTimer();
        }
    ...
    public int btAddServerResource(long j, int i, ServerResourceParam serverResourceParam) {
        if (serverResourceParam == null) {
            LogUtils.e("btAddServerResource serverResPara is null, task=[" + j + ":" + i + "]");
            return 9112;
        }
        LogUtils.d( "btAddServerResource beg, task=[" + j + ":" + i + "] mUrl=[" + serverResourceParam.mUrl + "] mRefUrl=[" + serverResourceParam.mRefUrl + "] mCookie=[" + serverResourceParam.mCookie + "] mResType=[" + serverResourceParam.mResType + "] mStrategy=[" + serverResourceParam.mStrategy + "]");
        if (serverResourceParam.checkMemberVar()) {
            try {
                increRefCount();
                if (this.mLoader == null) {
                    LogUtils.e("btAddServerResource mLoader is null, task=[" + j + ":" + i + "]");
                    return 9900;
                } else if (XLManagerStatus.MANAGER_RUNNING != mDownloadManagerState) {
                    LogUtils.e("btAddServerResource mDownloadManagerState is invaild, task=[" + j + ":" + i + "] mDownloadManagerState=[" + mDownloadManagerState + "]");
                    decreRefCount();
                    return 9900;
                } else {
                    ...
                    if (9000 != btAddServerResource) {
                        LogUtils.e("btAddServerResource btAddServerResource failed, task=[" + j + ":" + i + "] errno=[" + btAddServerResource + "]");
                        decreRefCount();
                        return btAddServerResource;
                    }
                    String str = TAG;
                    String str__ = "btAddServerResource end success, task=[" + j + ":" + i + "]";
                    LogUtils.d(str, str__);
                    decreRefCount();
                    return 9000;
                }
            } finally {
            ...
            LogUtils.e("btAddServerResource checkMemberVar failed, task=[" + j + ":" + i + "] mUrl=[" + serverResourceParam.mUrl + "] mRefUrl=[" + serverResourceParam.mRefUrl + "] mCookie=[" + serverResourceParam.mCookie + "]");
            return 9112;
        }
    }
    ...
    public int btAddPeerResource(long j, int i, PeerResourceParam peerResourceParam) {
        if (peerResourceParam == null) {
            LogUtils.e("btAddPeerResource peerResPara is null, task=[" + j + ":" + i + "]");
            return 9112;
        }
        LogUtils.d( "btAddPeerResource beg, task=[" + j + ":" + i + "] mPeerId=[" + peerResourceParam.mPeerId + "] mUserId=[" + peerResourceParam.mUserId + "] mJmpKey=[" + peerResourceParam.mJmpKey + "] mVipCdnAuth=[" + peerResourceParam.mVipCdnAuth + "] mInternalIp=[" + peerResourceParam.mInternalIp + "] mTcpPort=[" + peerResourceParam.mTcpPort + "] mUdpPort=[" + peerResourceParam.mUdpPort + "] mResLevel=[" + peerResourceParam.mResLevel + "] mResPriority=[" + peerResourceParam.mResPriority + "] mCapabilityFlag=[" + peerResourceParam.mCapabilityFlag + "] mResType=[" + peerResourceParam.mResType + "]");
        if (peerResourceParam.checkMemberVar()) {
            try {
                increRefCount();
                if (this.mLoader == null) {
                    LogUtils.e("btAddPeerResource mLoader is null, task=[" + j + ":" + i + "]");
                    return 9900;
                } else if (XLManagerStatus.MANAGER_RUNNING != mDownloadManagerState) {
                    LogUtils.e("btAddPeerResource mDownloadManagerState is invaild, task=[" + j + ":" + i + "] mDownloadManagerState=[" + mDownloadManagerState + "]");
                    decreRefCount();
                    return 9900;
                } else {
                    ...
                    if (9000 != btAddPeerResource) {
                        LogUtils.e("btAddPeerResource btAddPeerResource failed, task=[" + j + ":" + i + "] errno=[" + btAddPeerResource + "]");
                        decreRefCount();
                        return btAddPeerResource;
                    }
                    LogUtils.d( "btAddPeerResource end success, task=[" + j + ":" + i + "]");
                    decreRefCount();
                    return 9000;
                }
            } finally {
            ...
            LogUtils.e("btAddPeerResource peerResPara checkMemberVar failed, task=[" + j + ":" + i + "]");
            return 9112;
        }
    }
    ...
    private void loadErrcodeString(Context context) {
        if (context == null) {
            LogUtils.e("loadErrcodeString, context invalid");
        } else {
    ...
    public String getErrorCodeMsg(int i) {
        ...
        if (!(mErrcodeStringMap == null || num == null)) {
            ...
            LogUtils.d("errcode:" + i + ", errcodeMsg:" + str);
        }
        return str;
    }
    ...
    public int setSpeedLimit(long j, long j2) {
        LogUtils.d( "debug: XLDownloadManager::setSpeedLimit beg, maxDownloadSpeed=[" + j + "] maxUploadSpeed=[" + j2 + "]");
        if (this.mLoader == null) {
            LogUtils.e("error: XLDownloadManager::setSpeedLimit mLoader is null, maxDownloadSpeed=[" + j + "] maxUploadSpeed=[" + j2 + "] ret=[9900]");
            return 9900;
        }
        int speedLimit = this.mLoader.setSpeedLimit(j, j2);
        LogUtils.d( "debug: XLDownloadManager::setSpeedLimit end, maxDownloadSpeed=[" + j + "] maxUploadSpeed=[" + j2 + "] ret=[" + speedLimit + "]");
        return speedLimit;
    }

    public int setBtPriorSubTask(long j, int i) {
        LogUtils.d( "XLDownloadManager::setBtPriorSubTask beg, taskId=[" + j + "] fileIndex=[" + i + "]");
        if (this.mLoader == null) {
            LogUtils.e("XLDownloadManager::setBtPriorSubTask mLoader is null, taskId=[" + j + "] fileIndex=[" + i + "]");
            return 9900;
        }
        int btPriorSubTask = this.mLoader.setBtPriorSubTask(j, i);
        if (9000 != btPriorSubTask) {
            LogUtils.e("XLDownloadManager::setBtPriorSubTask end, taskId=[" + j + "] fileIndex=[" + i + "] ret=[" + btPriorSubTask + "]");
            return btPriorSubTask;
        }
        LogUtils.d( " XLDownloadManager::setBtPriorSubTask end, taskId=[" + j + "] fileIndex=[" + i + "]");
        return 9000;
    }

    public int getMaxDownloadSpeed(MaxDownloadSpeedParam maxDownloadSpeedParam) {
        if (this.mLoader == null) {
            LogUtils.e("XLDownloadManager::getMaxDownloadSpeed mLoader is null");
            return 9900;
        }
        int maxDownloadSpeed = this.mLoader.getMaxDownloadSpeed(maxDownloadSpeedParam);
        if (9000 != maxDownloadSpeed) {
            LogUtils.e("XLDownloadManager::getMaxDownloadSpeed end, ret=[" + maxDownloadSpeed + "]");
            return maxDownloadSpeed;
        }
        LogUtils.d( "XLDownloadManager::getMaxDownloadSpeed end, speed=[" + maxDownloadSpeedParam.mSpeed + "] ret=[" + maxDownloadSpeed + "]");
        return maxDownloadSpeed;
    }

    public int statExternalInfo(long j, int i, String str, String str2) {
        LogUtils.d( "XLDownloadManager::statExternalInfo beg, taskId=[" + j + "] fileIndex=[" + i + "] key=[" + str + "] value=[" + str2 + "]");
        if (this.mLoader == null) {
            LogUtils.e("XLDownloadManager::statExternalInfo mLoader is null, taskId=[" + j + "] fileIndex=[" + i + "]");
            return 9900;
        }
        int statExternalInfo = this.mLoader.statExternalInfo(j, i, str, str2);
        if (9000 != statExternalInfo) {
            LogUtils.e("XLDownloadManager::statExternalInfo end, taskId=[" + j + "] fileIndex=[" + i + "] ret=[" + statExternalInfo + "]");
            return statExternalInfo;
        }
        LogUtils.d( "XLDownloadManager::statExternalInfo end, taskId=[" + j + "] fileIndex=[" + i + "] ret=[" + statExternalInfo + "]");
        return statExternalInfo;
    }
    ...
    public int clearTaskFile(String str) {
        LogUtils.d( "XLDownloadManager::clearTaskFile filePath=[" + str + "]");
        if (TextUtils.isEmpty(str)) {
            return 9900;
        }
        if (this.mLoader == null) {
            LogUtils.e("XLDownloadManager::clearTaskFile mLoader is null");
            return 9900;
        }
        ...
        LogUtils.e("XLDownloadManager::clearTaskFile end, ret=[" + clearTaskFile + "]");
    ...
    public int startDcdn(long j, int i, String str, String str2, String str3) {
        ...
        decreRefCount();
        LogUtils.d( String.format("XLDownloadManager::startDcdn ret=[%d] taskId=[%d] subIndex=[%d] sessionId=[%s] productType=[%s] verifyInfo=[%s]", new Object[]{Integer.valueOf(i2), Long.valueOf(j), Integer.valueOf(i), str, str2, str3}));
        return i2;
    }
    ...
    public int stopDcdn(long j, int i) {
        ...
        decreRefCount();
        LogUtils.d( String.format("XLDownloadManager::stopDcdn ret=[%d] taskId=[%d] subIndex=[%d]", new Object[]{Integer.valueOf(i2), Long.valueOf(j), Integer.valueOf(i)}));
        return i2;
    }

    public int createShortVideoTask(String str, String str2, String str3, String str4, int i, int i2, int i3, GetTaskId getTaskId) {
        LogUtils.d( "XLDownloadManager::createShortVideoTask beg, url=[" + str + "] path=[" + str2 + "] filename=[" + str3 + "] title=[" + str4 + "]");
        if (this.mLoader == null) {
            LogUtils.e("XLDownloadManager::createShortVideoTask mLoader is null");
            return 9900;
        }
        String str5;
        if (str4 == null) {
        ...
        if (9000 != createShortVideoTask) {
            LogUtils.e("XLDownloadManager::createShortVideoTask end, ret=[" + createShortVideoTask + "]");
            return createShortVideoTask;
        }
        LogUtils.d( "XLDownloadManager::createShortVideoTask end, taskId=[" + getTaskId.getTaskId() + "] ret=[" + createShortVideoTask + "]");
        return createShortVideoTask;
    }

    public int playShortVideoBegin(long j) {
        LogUtils.d( "XLDownloadManager::playShortVideoBegin beg, taskId=[" + j + "]");
        if (this.mLoader == null) {
            LogUtils.e("XLDownloadManager::playShortVideoBegin mLoader is null");
            return 9900;
        }
        int playShortVideoBegin = this.mLoader.playShortVideoBegin(j);
        if (9000 != playShortVideoBegin) {
            LogUtils.e("XLDownloadManager::playShortVideoBegin end, ret=[" + playShortVideoBegin + "]");
            return playShortVideoBegin;
        }
        LogUtils.d( "XLDownloadManager::playShortVideoBegin end, taskId=[" + j + "] ret=[" + playShortVideoBegin + "]");
        return playShortVideoBegin;
    }

    public int getSessionInfoByUrl(String str, XLSessionInfo xLSessionInfo) {
        if (this.mLoader == null) {
            LogUtils.e("XLDownloadManager::getSessionInfoByUrl mLoader is null");
            return 9900;
        }
        ...
        LogUtils.e("XLDownloadManager::getSessionInfoByUrl end, ret=[" + sessionInfoByUrl + "]");
        return sessionInfoByUrl;
    }
    ...
    class NetworkChangeHandlerThread implements Runnable {
        ...
        public void run() {
            if (this.m_allow_execution) {
                ...
                LogUtils.d("XLDownloadManager", "NetworkChangeHandlerThread nettype=" + networkTypeComplete);
                this.this$0.notifyNetWorkType(networkTypeComplete, this.m_loader);
                String bssid = XLUtil.getBSSID(this.m_context);
                LogUtils.d("XLDownloadManager", "NetworkChangeHandlerThread bssid=" + bssid);
                ...
                LogUtils.d("XLDownloadManager", "NetworkChangeHandlerThread NetWorkCarrier=" + netWorkCarrier);
